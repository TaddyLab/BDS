...
# Now we need to create a sparse matrix for modeling.
# Recall that a sparse matrix provides an efficient way to store
# data with lots of zeros. If we think in terms of a regular matrix,
# we want to store the amount of time spent by a particular machine at a partiuclar site.
# Thus, if we are constructing this regular matrix, our row axis (i axis)
# describes a particular machine and the column axis (j axis) describes
# a particular website. So, item i,j in this regular matrix describes the
# amount of time spent by machine i at website j.
# Creating a sparse matrix of this is simply, transferring this description
# into the sparse matrix code. We show that now.
