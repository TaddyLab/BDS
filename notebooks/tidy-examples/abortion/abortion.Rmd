...
Now, instead stacking $\tilde{d}$ and $\tilde{y}$ across folds and then running the regression, we follow Chernozhukov et al., (2018) and define the cross-fitting estimator for $\gamma$ as the average $\gamma$ over multiple splits of the data

$$
\widehat{\gamma}_{CF}=\frac{1}{K} \sum_{k=1}^{K} \widehat{\gamma}_{(k)}
$$
and the estimator for the variance as
